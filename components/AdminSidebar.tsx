...
const roles: Record<Role, NavItem[]> = {
  ...
  technician: [
    { name: "addDevice", path: "/technician/add-device" },
    { name: "removeDevice", path: "/technician/remove-device" },
    { name: "maintainDevice", path: "/technician/maintain-device" },
  ],
};
...
export default function Layout({ role = "main", user = {name: "Asheanfi"}}: LayoutProps) {
  return (
    <Navigation role={role} user={user}/>  
  );
}

function Navigation({ role = "main", user }: { role: Role; user?: User }) {
