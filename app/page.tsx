...
// app/thingspeak/page.tsx  OR  pages/thingspeak.tsx

'use client'; // Use this line only in App Router (`app/` directory)
...
interface ThingSpeakFeed {
  ...
  field1?: string;
  field2?: string;
  // Add more fields if needed (e.g., field3, field4)
}
    ...
    async function fetchData() {
      try {
        const res = await fetch(
          'https://api.thingspeak.com/channels/2956216/feeds.json?results=1'
        );
    ...
    <main className="p-6 max-w-xl mx-auto">
      ...
      {data && data.feeds.length > 0 && (
        <div className="border p-4 rounded-lg shadow-md space-y-2">
          <p><strong>Timestamp:</strong> {data.feeds[0].created_at}</p>
          <p><strong>Field 1:</strong> {data.feeds[0].field1 ?? 'N/A'}</p>
          <p><strong>Field 2:</strong> {data.feeds[0].field2 ?? 'N/A'}</p>
        </div>
      )}
    </main>
  );
